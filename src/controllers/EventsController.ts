...
EventsController.get("/latest", async (req, res) => {
	const result = await db.select().from(VEHICLE_EVENTS).orderBy(desc(VEHICLE_EVENTS.created_at)).limit(500);
	res.json({
		success: true,
		data: result,
	});
...
EventsController.get("/list", async (req, res) => {
	try {
		...
		const result = await db
			.select()
			.from(VEHICLE_EVENTS).orderBy(asc(VEHICLE_EVENTS.created_at))
			.offset(page * limit)
			.limit(limit)
		res.json({
			success: true,
	...
	try {
		...
		if (!(route_code)||![0,1].includes(direction)) {
			res.json({
				success:false,
				data:null,
				message:"direction must be either 0 or 1, and route code must be valid"
			})
			return
		}
		const result = await db.select().from(VEHICLE_EVENTS)
			...
			.where(
				and(
					...
					and(eq(VEHICLE_EVENTS.route_code, route_code), eq(VEHICLE_EVENTS.direction, direction))
				)
			)
			.limit(500)
		res.json({
...
EventsController.get("/search", async (req, res) => {
	try {
		...
		let prepared
		if (search_key === "route") {
			prepared = db.select().from(VEHICLE_EVENTS)
				.orderBy(desc(VEHICLE_EVENTS.created_at))
