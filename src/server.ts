...
import db from "./db";
import { SUBSCRIBERS, VTS, VTS_METADATA } from "./schema";
...
SERVER.get("/realtime", async (req, res) => {
	const [latest_entry_metadata] = await db
		.select()
		.from(VTS_METADATA)
		.orderBy(desc(VTS_METADATA.created_at))
		.limit(1);
	const latest_key = latest_entry_metadata.created_at;
	const latest_data = await db
		.select()
		.from(VTS)
		.where(eq(VTS.created_at, latest_key));
	res.json({
		metadata: latest_entry_metadata,
		data: latest_data,
	});
});
// github copilot slop
