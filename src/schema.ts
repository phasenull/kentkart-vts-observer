...
export const VTS = sqliteTable(
	...
	(table) => [
		index("vts_vehicle_id_index").on(table.vehicle_id, table.created_at),
...
export const MESSAGES = sqliteTable(
	"messages",
	{
		...
		sent_at: int({ mode: "timestamp" }).notNull(),
		content: text().notNull(),
	},
	(table) => [
		index("messages_chat_id_index").on(table.chat_id),
		unique("messages_unique_index").on(table.chat_id, table.message_id),
	]
);
...
export const TRIPS = sqliteTable("trips", {
	...
	branched_display_route_code: text("branched_display_route_code", {
		length: 10,
	}),
...
export const VEHICLE_EVENTS = sqliteTable(
	"vehicle_events",
	{
		created_at: int("created_at", { mode: "timestamp" }).notNull(),
		...
		event_label: text("event_label")
			.$type<"vehicle_created" | "vehicle_destroyed">()
			.notNull(),
	},
	(table) => [
